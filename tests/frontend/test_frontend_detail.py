...
class FrontendDetailTests(unittest.TestCase):
    def test_vacancy_detail_exposes_required_match_and_job_fields(self) -> None:
        ...
        for label in ("Description", "Estimated salary", "Location", "Work arrangement", "Compatibility", "Detected gaps", "Recommendations"):
    ...
    def test_external_links_are_explicit_and_safe(self) -> None:
        matches_apply = re.findall(r'<a[^>]+>Apply', APP)
        matches_original = re.findall(r'<a[^>]+>View original description', APP)
        self.assertEqual(len(matches_apply), 1)
        ...
        self.assertIn("View original description", APP)
    ...
    def test_detail_can_return_to_list_and_is_keyboard_accessible(self) -> None:
        ...
        self.assertIn("← Back to openings", APP)
